...
Open **Overview > Import PDF / Excel statement**, enter your names, UPI IDs, or account last four digits, then choose a text-based PDF or `.xlsx` file. The importer detects likely counterparties from available columns and transaction text, so it does not require a specifically named Receiver column. Similar transactions are grouped by normalized counterparty ID/name—not amount. Groups matching your identifiers remain visible but start unchecked, and category changes apply to the whole group. Duplicate imports are skipped. Scanned PDFs and legacy `.xls` files are not supported.
