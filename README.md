...
Open **Overview > Import PDF / Excel statement**, enter your names, UPI IDs, or account last four digits, then choose a text-based PDF or `.xlsx` file. For PDFs, the importer preserves table positions, identifies sender/from and receiver/to column roles from headers, determines whether each transaction is outgoing, and falls back to directional narration markers such as payee, beneficiary, merchant, or “paid to.” Sender details are never used as the receiver. Transactions are grouped by normalized receiver ID/name—not amount. Matching receiver groups remain visible but start unchecked, and category changes apply to the whole group. Rows without a confident receiver are marked **Needs review**. Duplicate imports are skipped. Scanned PDFs and legacy `.xls` files are not supported.
